...
public class dischargePatient extends JFrame {
    ...
    private JPanel createButtonPanel() {
        ...
        JButton dischargeButton = createStyledButton("Discharge Patient", SUCCESS);
    ...
    private void dischargePatient() {
        ...
        if (selectedRow >= 0) {
            ...
            try {
                String query = "DELETE FROM patient_record WHERE ID = ?";
                try (PreparedStatement pstmt = connection.prepareStatement(query)) {
                    pstmt.setInt(1, patientId);
                    pstmt.executeUpdate();
                }
                loadData(true); // Reload data after discharge
                showSuccess("Patient discharged successfully.");
            } catch (SQLException e) {
                showError("Discharge Error", e.getMessage());
            }
        } else {
            showError("Selection Error", "Please select a patient to discharge.");
        }
    }
    ...
    private void handleBack() {
        // Handle back action (return to previous window)
